...
#设定 端口
port_alterid_set(){
	echo -e "${Info} ${GreenBG} 请输入连接端口（默认:8080 无特殊需求请直接按回车键） ${Font}"
...
apache_uninstall(){
	...
	rm -rf /usr/local/bin/cloud-torrent /usr/local/bin/downloads /usr/local/bin/cloud-torrent.json /etc/systemd/system/cloud-torrent.service >/dev/null 2>&1
...
ExecStart=/usr/local/bin/cloud-torrent --port ${port} --title "Cloud Torrent"
Restart=always
RestartSec=3
